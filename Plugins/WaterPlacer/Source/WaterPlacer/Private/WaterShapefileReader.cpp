...
namespace
{
	...
	bool ValidateEpsg4326Prj(const FString& PrjPath, FString& OutError)
	{
		...
		{
			UE_LOG(LogWaterPlacer, Display, TEXT("No .prj found; assuming EPSG:4326 (WGS84 lon/lat degrees)."));
			return true;
		}
		...
		{
			UE_LOG(LogWaterPlacer, Warning, TEXT("Could not read .prj; assuming EPSG:4326."));
			return true;
		}
		...
		if (bLooks4326)
		{
			UE_LOG(LogWaterPlacer, Display, TEXT("CRS check OK: .prj looks like EPSG:4326 / WGS84."));
			return true;
		}
...
bool WaterShapefileReader::ReadWaterPolygons(
	...
	FString& OutError)
{
	...
	else if (!AltitudeFieldName.IsEmpty())
	{
		OutError = FString::Printf(TEXT("DBF not found (needed for altitude field '%s'): %s"), *AltitudeFieldName, *DbfPath);
		return false;
	}
	...
	{
		ElevField = FindField(DbfFields, AltitudeFieldName);
		if (!ElevField)
		{
	...
	while (Offset + 8 <= ShpData.Num())
	{
		...
		FWaterShapefilePolygon Feature;
		...
		if (ElevField && RecordIndex < DbfRecords.Num())
		{
			...
			if (Rec.Num() > 0 && Rec[0] != '*')
			{
				ParseNumericField(Rec, *ElevField, Feature.AltitudeM);
			}
		}

		if (Feature.OuterRingLonLat.Num() >= 3)
		{
			OutPolygons.Add(MoveTemp(Feature));
		}
	...
	if (OutPolygons.Num() == 0)
	{
		OutError = TEXT("No valid water polygons found in shapefile.");
		return false;
	}
